...
export function AppHeaderUser({
  ...
  const pageMiningStatus: React.MutableRefObject<boolean> = useRef(false);

  const handleMineButtonClick = () => {

    // Invert page-wide mining & mining-button status.
    const newMiningStatus = !pageMiningStatus.current;

    // Update "MINE" button state (On / Off);
    pageMiningStatus.current = newMiningStatus;

    // Update Site-Wide Mining Logic
    pageMiningHandler(newMiningStatus);

    // Set loading state to true when the button is clicked
    setIsLoading(true);

    if (newMiningStatus === true) {
  ...
  return (
    <div className="App-header-user">
      ...
      >
        ...
        >
          <img src={UnionIcon} width={16} alt="" />
